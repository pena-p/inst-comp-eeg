...
function events = eeg_behav_match(cfg,subject,events)
id = subject.id;
...
for b = 1:cfg.exp.nblocks
    if block_seq(b) < 3
        ...
        for t = 1:height(block)
            if t == 1
                events_behav{end+1} = ['S  ' num2str(block_seq(b))];
            ...
            for e = {'Inst_1_', 'ISI_Inst_1_',...
                ...
                if and(strcmp(e{1},'PROBE_'),min(block.RT{t})<0.2)
                    events_behav{end+1} = 'Response';
                    blockID{end+1} = ['TASK_' num2str(run_cnt)];
                    trialID{end+1} = num2str(t);
                    accuracy{end+1} = block.acc(t);
                    RT{end+1} = block.RT{t};
                ...
                elseif and(strcmp(e{1},'PROBE_FIX_'),min(block.RT{t})>0.2)
                    events_behav{end+1} = 'Response';
                    blockID{end+1} = ['TASK_' num2str(run_cnt)];
                    trialID{end+1} = num2str(t);
                    accuracy{end+1} = block.acc(t);
    ...
    elseif block_seq(b) == 3
        ...
        for t = 1:height(block)
            ...
            for e = {'PROBE_', 'PROBE_FIX_'}
                ...
                if and(strcmp(e{1},'PROBE_'),block.RT{t}<0.2)
                    events_behav{end+1} = 'Response';
                    blockID{end+1} = ['LOC_' num2str(run_cnt)];
                    trialID{end+1} = num2str(t);
                    accuracy{end+1} = block.acc(t);
                    RT{end+1} = block.RT{t};
                    catch_T{end+1} = nan;
                ...
                elseif and(strcmp(e{1},'PROBE_FIX_'),block.RT{t}>0.2)
                    events_behav{end+1} = 'Response';
                    blockID{end+1} = ['LOC_' num2str(run_cnt)];
                    trialID{end+1} = num2str(t);
                    accuracy{end+1} = block.acc(t);
