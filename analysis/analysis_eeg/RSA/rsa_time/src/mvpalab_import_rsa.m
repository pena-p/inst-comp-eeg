function [cfg, data, fv] = mvpalab_import_rsa(cfg)
...
nSubjects = 1:length(cfg.study.dataFiles{1,1});
nClasses = length(cfg.study.conditionIdentifier);
nCtxt = 1;
...
if strcmp(cfg.analysis,'RSA_the')|| strcmp(cfg.analysis,'RSA_pearson_cv')
    nCtxt = 1;
...
for sub = nSubjects
    ...
    for freq = 1 : cfg.sf.nfreq
        ...
        if cfg.sf.flag
            [fv{sub},cfg] = mvpalab_fext_rsa(cfg,data(sub,:),sub);
