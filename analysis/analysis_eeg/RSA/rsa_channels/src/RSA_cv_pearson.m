...
for i = 1:length(fv)
    ...
    for chan = 1:electrodes
        text = ['Computing subject ' int2str(i) ' on channel ' int2str(chan) '/' int2str(electrodes)];
        fprintf(text);
        correl_matrix = cross_validated_pearson(squeeze(Z(:,chan,cfg.tm.tpoints)), labels, folds);
